...
proxies = {
    "http": "http://sp9ptvdbhx:p11HxEldXudLxu02i_@gate.decodo.com:7000",
    "https": "http://sp9ptvdbhx:p11HxEldXudLxu02i_@gate.decodo.com:7000",
}
major_proxies = {
    "http": "http://sp9ptvdbhx:p11HxEldXudLxu02i_@gate.decodo.com:7000",
    "https": "http://sp9ptvdbhx:p11HxEldXudLxu02i_@gate.decodo.com:7000",
...
# ---------------- NAME / PASSWORD ---------------- #
def generate_random_name(_=None):
    return "Adam" + ''.join(random.choice(string.digits) for _ in range(6))


def generate_custom_password():
    characters = string.ascii_letters + string.digits
    random_part = ''.join(random.choice(characters) for _ in range(12)).upper()
    return f"ADAM_{random_part}_CORE"
...
def create_acc(region):
    ...
    try:
        ...
        return f"ERROR: IN create_acc {e}."
...
def token(uid, password, region):
    ...
    try:
        ...
        response = session.post(url, headers=headers, data=body, proxies=proxies, verify=False)
        resp_json = response.json()
        ...
        if not open_id or not access_token:
            print(resp_json)
            return f"ERROR: NO ACCESS TOKEN. {resp_json}"
...
def Major_Regsiter(access_token, open_id, field, uid, password, region):
    ...
    internal_name = generate_random_name()
    ...
    try:
        ...
        response = session.post(url, headers=headers, data=body, proxies=major_proxies, verify=False)
        ...
        return f"ERROR: IN Major_Regsiter {e}"

def chooseregion(jwt_token, region):
...
def login(uid, password, access_token, open_id, response_hex, status_code, name, region):
    ...
    if RESPONSE.status_code == 200:
        ...
        if lang.lower() not in ["ar", "en"]:
            ...
            if BASE64_TOKEN:
                r = chooseregion(BASE64_TOKEN, "ME")
                if r == 200:
                    result = {
                        "uid": uid,
                        "password": password,
                    ...
                    save_account_to_file(result)
                    return result #login_server(uid, password, access_token, open_id, RESPONSE.content.hex(), RESPONSE.status_code, name, region)
                else:
                    return "hi1"
...
def login_server(uid, password, access_token, open_id, response, status_code, name, region):
    ...
    try:
        session = get_session()
        RESPONSE = session.post(URL, headers=headers, data=Final_Payload, proxies=proxies, verify=False)
    except Exception:
        return None
...
def generate_accounts(count, region):
    ...
    print(f"Starting creation of {count} accounts for region {region} with name prefix AdamBot\n")
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=20) as executor:
        futures = [executor.submit(create_single_account, region) for _ in range(count)]
        for future in concurrent.futures.as_completed(futures):
            attempts += 1
            result = future.result()
            if result and result.get('status') == "full_login":
                results.append(result)
                print(bg_green + f"Successfully created the account {len(results)}/{count}: UID {result['uid']}" + reset)
...
#print(generate_accounts("10", "me"))
count = 1
while True:
    result = create_acc("me")
    if result is not None and "ERROR" not in result and "BANNED" not in result:
